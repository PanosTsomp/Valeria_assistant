...
export default function AudioPage(): React.JSX.Element {
  const {
    isRecording, recordError, audioStatus, savedFilePath,
    isPlaying, transcript, isTranscribing, transcribeTime,
    handleToggleRecording, handlePlayback,
  } = useTranscription()
  ...
  return (
    <>
      ...
      <div style={sectionStyle}>
        ...
        {audioStatus && (
          <div style={{ ...infoBoxStyle, color: '#90caf9' }}>{audioStatus}</div>
        )}
        {isTranscribing && (
      ...
      <div style={sectionStyle}>
        ...
        {ttsError && (
          <div style={{ ...infoBoxStyle, color: '#ef5350' }}>TTS Error: {ttsError}</div>
        )}
      </div>
    </>
  )
